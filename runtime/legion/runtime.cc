...
namespace LegionRuntime {
  ...
  namespace HighLevel {
	...
	machine.get_all_processors(all_procs);
        ...
        manager->add_mapper(0, new WrapperMapper(new DefaultMapper(machine, high_level, *it) ,machine, high_level, *it),
                            false/*needs check*/);
      }
...
#undef BOOL_ARG
    ...
    //--------------------------------------------------------------------------
    {
			WrapperMapper::task_names_map[uid]=std::string(name);
